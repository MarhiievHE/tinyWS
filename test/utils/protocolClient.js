...
// Events: 'open', 'message'(Buffer), 'frame'(opcode, Buffer), 'ping'(Buffer), 'pong'(Buffer), 'close'(code, reason)
// Methods: send(data), sendFrame(opcode, payload, { fin=true, mask=true }), sendText, sendBinary, ping, close
class ProtocolClient {
  ...
  // Convenience: send string as text, Buffer/TypedArray/ArrayBuffer as binary; others stringified
  send(data, opts) {
  ...
  // Options: { host, port, path, headers, method, httpVersion, includeHost, timeoutMs }
