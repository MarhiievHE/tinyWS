...
class Connection extends EventEmitter {
  ...
  #processFrameParserError(error) {
    ...
    this.#close(Frame.errorClose(type, subtype, this.#isClient));
  }
  ...
  #handleDataFrame(frame) {
    ...
    if (!this.#fragments) {
      if (frame.fin) {
        ...
        if (
          opcode === OPCODES.TEXT &&
          !this.#utf8Validator.push(payload, fin)
        ) {
          this.emit(
            'error',
          ...
          return void this.#close(
            Frame.protocolErrorClose('INVALID_PAYLOAD', this.#isClient),
          );
        }
      ...
      if (!isBinary && !this.#utf8Validator.push(payload, fin)) {
        this.emit(
          'error',
          new Error('Invalid UTF-8 in text frame during fragments'),
        );
        return void this.#close(Frame.protocolErrorClose('INVALID_PAYLOAD'));
      }
      this.#fragments.payloads.push(frame.payload);
      if (!frame.fin) return;
      const fullPayload = Buffer.concat(this.#fragments.payloads);
      const message = isBinary ? fullPayload : fullPayload.toString();
  ...
  #fastPing() {
    this.#socket.write(Frame.emptyPingBuffer(this.#isClient));
  }

  #fastPong() {
    this.#socket.write(Frame.emptyPongBuffer(this.#isClient));
  }
  ...
  #close(frameBuffer) {
    ...
    try {
      ...
      // ignore write errors — we'll still attempt to close
    }
