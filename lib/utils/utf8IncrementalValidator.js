...
class Utf8IncrementalValidator {
  constructor() {
    ...
    this.firstContCheck = 0; // 0 = none, 1 = check range for the first continuation byte
   ...
   * @param {boolean} [fin=false] - set true for the last fragment of the message
   * @returns {boolean} true if valid so far; false if invalid (validator becomes closed for further use)
   */
  push(buf, fin = false) {
    ...
    while (i < len) {
      ...
      if (need > 0) {
        ...
        // If we have a pending constraint for the first continuation byte (E0/ED/F0/F4 cases), enforce it
        if (firstContCheck === 1) {
